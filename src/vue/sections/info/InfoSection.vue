    ...
    <SectionTemplate :section-data="props.sectionData">
        <div class="info-row row gx-4 gx-xl-5">
            <div v-for="subcategory in props.sectionData['content']['subcategories']" class="col-12 subcategory-col">
                <!-- Verifica se o ID é 1 e renderiza 'aaaa' -->
                <template v-if="subcategory.id === 'programming'">
                    <SubHeading v-if="subcategory['locales']['title']" :title="subcategory['locales']['title']"
                        :fa-icon="subcategory['faIcon']" />
                        
                    <div style="font-family: Arial, sans-serif; margin-bottom: 1em;">
                        <div class="programmingTitle" style="font-size: 1.3em; font-weight: bold; margin-bottom: 0.5em;">
                            <span class="ms-1">{{ data.getString('workingWith') }}</span>
                        </div>
                        <img src="https://skillicons.dev/icons?i=ruby,rails,vue,nuxt,typescript" class="select-none"
                            style="max-width: 100%;  height: auto;" />
                    </div>

                    <div style="font-family: Arial, sans-serif; margin-bottom: 1em;">
                        <div class="programmingTitle" style="font-size: 1.3em; font-weight: bold; margin-bottom: 0.5em;">
                            <span class="ms-1">{{ data.getString('experienceWith') }}</span>
                        </div>
                        <img src="https://skillicons.dev/icons?i=js,react,next,tailwind,docker" class="select-none mb-2"
                            style="max-width: 100%; display: flex;" />
                        <img src="https://skillicons.dev/icons?i=linux,mongodb,postgres,git,github"
                            class="select-none mb-2" style="max-width: 100%; height: auto;" />
                    </div>
                </template>

                <!-- Para todas as outras subcategorias -->
                <template v-else>
                    <!-- Subcategory Title -->
                    <SubHeading v-if="subcategory['locales']['title']" :title="subcategory['locales']['title']"
                        :fa-icon="subcategory['faIcon']" />
...
.subcategory-col {
    &:not(:last-child) {
        @include generate-dynamic-styles-with-hash((xxxl: (margin-bottom: 2.75rem),
